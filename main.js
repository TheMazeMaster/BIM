...
function drawTier(svg, tierConfig, tierIndex, cx, cy, rotationOffset) {
  if (!tierConfig.show) return;
  if (tierConfig.type === 'centered') drawCenteredText(svg, tierConfig, cx, cy);
  if (tierConfig.type === 'arcText') drawArcText(svg, tierConfig, cx, cy);
  if (tierConfig.type === 'radial') drawRadialTier(svg, tierConfig, tierIndex, cx, cy, rotationOffset);
}
